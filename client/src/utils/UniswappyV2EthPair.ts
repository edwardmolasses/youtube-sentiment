...
import { UNISWAP_LOOKUP_CONTRACT_ADDRESS_MAINNET, WETH_ADDRESS } from "./addresses";
...
export class UniswappyV2EthPair extends EthMarket {
  ...
  private UNISWAP_LOOKUP_CONTRACT_ADDRESS: string;
  ...
  constructor(marketAddress: string, tokens: Array<string>, protocol: string) {
    ...
    this.UNISWAP_LOOKUP_CONTRACT_ADDRESS = UNISWAP_LOOKUP_CONTRACT_ADDRESS_MAINNET;
  }
  ...
  public getTokenBalances(): TokenBalances {
  ...
  static async getUniswappyMarkets(
    ...
    uniswapLookupContractAddress: string = UNISWAP_LOOKUP_CONTRACT_ADDRESS_MAINNET
  ): Promise<Array<UniswappyV2EthPair>> {
    const uniswapQuery = new Contract(uniswapLookupContractAddress, UNISWAP_QUERY_ABI, provider);
    ...
    for (let i = 0; i < BATCH_COUNT_LIMIT * UNISWAP_BATCH_SIZE; i += UNISWAP_BATCH_SIZE) {
      ...
      // console.log(`pairs (${i}): `); console.log(pairs);
  ...
  static async getUniswapMarketsByToken(
    ...
    console.log('getUniswapMarketsByToken ...');
    const allPairs = await Promise.all(
      _.map(factoryAddresses, (factoryAddress) =>
        UniswappyV2EthPair.getUniswappyMarkets(provider, factoryAddress, uniswapLookupContractAddress)
      )
    );
  ...
  async sellTokens(tokenIn: string, amountIn: BigNumber, recipient: string): Promise<string> {
    ...
    const populatedTransaction = await UniswappyV2EthPair.uniswapInterface.populateTransaction.swap(
      amount0Out,
      amount1Out,
      recipient,
      []
